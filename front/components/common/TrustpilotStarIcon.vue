...
withDefaults(
  defineProps<{
    /** `em` scales with parent font size; `md` matches the Reviews rating row */
    scale?: "em" | "md";
  }>(),
  { scale: "md" },
);
...
<template>
  <span
    ...
    :class="scale === 'em' ? 'h-[1em] w-[1em] align-[-0.1em]' : 'h-5 w-5'"
  ...
  >
    <svg
      ...
      aria-hidden="true"
      :class="scale === 'em' ? 'h-[0.7em] w-[0.7em] fill-white' : 'h-3 w-3 fill-white'"
    >
      <path
