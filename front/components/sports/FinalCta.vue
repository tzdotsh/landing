    ...
    <div class="container relative">
      ...
      >
        <div class="relative mx-auto flex max-w-[560px] flex-col items-center gap-y-6">
          ...
          >
            <label class="sr-only" for="sports-final-email">
              {{ t("sports.final_cta.email_placeholder") }}
            </label>
            <input
              id="sports-final-email"
              v-model="email"
              type="email"
              name="email"
              autocomplete="email"
              inputmode="email"
              :placeholder="t('sports.final_cta.email_placeholder')"
              :aria-invalid="emailInvalid"
              :aria-describedby="emailInvalid ? 'sports-final-email-error' : undefined"
            ...
            >
              {{ t("sports.final_cta.cta_primary") }}
            </Button>
          </form>

          <p
            v-if="emailInvalid"
            id="sports-final-email-error"
            role="alert"
            class="text-on-accent/90 w-full text-left text-[12px]/[15px]"
          >
            {{ t("sports.final_cta.email_error") }}
          </p>
          ...
          >
            {{ t("sports.final_cta.cta_secondary") }}
          </a>
        </div>
      </div>
    </div>
